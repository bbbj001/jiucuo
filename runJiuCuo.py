...
def main():
    ...
    if not os.path.exists(outdir+'/raw.bam'):
        print("Generate bam")
        raw_cmd = "minimap2 -t32 -ax map-hifi %s %s | samtools view -@ 48 -bS > %s/raw.bam"%(ref,reads_path,outdir)
        raw_p=subprocess.Popen(raw_cmd,shell=True)
        raw_code=raw_p.wait()  #等待子进程结束，并返回状态码；
        print("Done!")
